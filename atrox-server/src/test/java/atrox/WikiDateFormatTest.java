...
import atrox.main.wiki.WikiDateFormat;
...
public class WikiDateFormatTest {
	...
	@Test public void testDateParsing () throws Exception {
		...
		for (String[] test : TESTS) {
            ...
            final TimePoint[] times;
            boolean parsedOk = false;
            try {
                times = WikiDateFormat.parse(input);
                parsedOk = true;
                String output = times[0].toString();
                if (times[1] != null && !times[0].equals(times[1])) output += "_" + times[1].toString();
